...
MODEL='/disk2/DOTA/snapshots/resnet50-900/resnet50_csv_30.h5'
...
model = models.load_model(MODEL, backbone_name="resnet50",convert=True)
st3 = time.time()
...
fl=glob.glob("/disk2/DOTA/validation/images/*.png")
totalCrops=0
totalTime=0
totalTim=0
for fn in fl:
    print(fn,fn+'.det.jpg')
    if not os.path.exists(fn+'.det.jpg'):
        img=cv2.imread(fn)
        print(img.shape)
        st = time.time()
        dets,crops,tim=get_detections(img, model)
        st1 = time.time()
        ...
        print('Clear inference time = {}'.format(tim))
        print('Inference per crop = {}'.format((st1 - st)/crops))
        print('Clear inference per crop = {}'.format(tim/crops))
...
print('Total clear inference time = {}'.format(totalTim))
print('Average inference per crop = {}'.format(totalTime/totalCrops))
print('Average clear inference per crop = {}'.format(totalTim/totalCrops))
